...
func ClaudeToKiro(req *ClaudeRequest, thinkingPrompt string) *KiroPayload {
	...
	// 含图片的 tool result 必须保留结构化以承载图片，即使没有匹配的 assistant tool 调用。
...
func sanitizeKiroHistory(history []KiroHistoryMessage, currentToolResultIDs map[string]bool) []KiroHistoryMessage {
	...
	for i := range history {
		...
		if msg.UserInputMessage != nil && msg.UserInputMessage.UserInputMessageContext != nil {
			...
			// 携带图片的 tool result 保留结构化（图片依赖结构化 tool result 承载，
			// 展平为文本会丢失图片）；其余照常叙述为文本。
...
func convertOpenAITools(tools []OpenAITool) []KiroToolWrapper {
	...
	for _, tool := range tools {
		...
		name := shortenToolNameWithMap(tool.Function.Name)
